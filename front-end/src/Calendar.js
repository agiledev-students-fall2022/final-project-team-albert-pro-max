...
class Calendar extends Component {
  ...
  getMonday() {
    ...
    let mondayDate = new Date(today.setDate(diff));
    let mondayDateStr = mondayDate.toISOString().substring(0, 10) + "T00:00:00";
    ...
    axios.get(`${this.BASE_URL}/schedule`, {
      ...
      .then(response => {
        ...
        response.data.map((e, index) => {
          ...
          let sTime = new DayPilot.Date(this.getMonday()).addHours(startH).addMinutes(startM);
          let eTime = new DayPilot.Date(this.getMonday()).addHours(endH).addMinutes(endM);
          ...
          if (e.rec) {
            ...
            if (time[4] === "PM" && end[0] !== "12") endH += 12;
            endM = parseInt(end[1]);
            sTime = new DayPilot.Date(this.getMonday()).addHours(startH).addMinutes(startM);
            eTime = new DayPilot.Date(this.getMonday()).addHours(endH).addMinutes(endM);
            let startTime = sTime;
            let endTime = eTime;
            let d = item.days;
            if (d === "Mon") {
